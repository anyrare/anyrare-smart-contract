...
contract AssetFactoryFacet {
    ...
    function currency() private view returns (IERC20) {
        require(
            ...
            "AssetFactoryFacet: currency address cannot be 0"
        );
    ...
    function asset() private view returns (AssetFacet) {
        require(
            ...
            "AssetFactoryFacet: assetDiamond address cannot be 0"
        );
    ...
    function openAuction(
        ...
        currency().transferFrom(msg.sender, address(this), fee);
        currency().transfer(LibData.getReferral(s, msg.sender), referralFee);
    ...
    function bidAuction(
            ...
            info.isAuction &&
                ...
                (
                    auction.bidder != msg.sender
                        ? currency().balanceOf(msg.sender) >= maxBid
        ...
        if (maxBid <= auction.maxBid) {
            ...
            if (
                ...
                currency().transfer(auction.bidder, auction.maxBid);
            }
    ...
    function openOffer(uint256 bidValue, uint256 tokenId) external {
        ...
        if (info.isOffer && info.offerBidder != msg.sender) {
            currency().transfer(info.offerBidder, info.offerValue);
        }
    ...
    function revertOffer(uint256 tokenId) external {
        ...
        currency().transferFrom(address(this), info.offerBidder, info.offerValue);
        asset().updateOfferStatus(tokenId, false);
    }
    ...
    function transferOpenFee(
        ...
        require(currency().balanceOf(msg.sender) >= platformFee + referralFee);
