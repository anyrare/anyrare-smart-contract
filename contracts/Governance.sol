...
contract Governance {
  ...
  constructor() public {
    Policy storage collateralWeight =
        policies[stringToBytes8("COLLATERAL_WEIGHT")];
  ...
  function initPolicy(string memory policyName, uint32 policyWeight,
                      uint32 maxWeight, uint32 voteDurationSecond,
                      uint32 minWeightOpenVote, uint32 minWeightValidVote,
                      uint32 minWeightApproveVote) public {
    require(isManager(msg.sender), "Error 3000: No permission to init policy.");

    bytes8 policyIndex = stringToBytes8(policyName);
    require(!policies[policyIndex].exists,
  ...
  function setPolicyByProposal(
      bytes8 policyIndex, address proposalAddress, uint32 policyWeight,
      uint32 maxWeight, uint32 voteDurationSecond, uint32 minWeightOpenVote,
      uint32 minWeightValidVote, uint32 minWeightApproveVote) public {
    require(msg.sender == proposalContract,
            "Error 3002: No permission to set policy.");

    Policy storage p = policies[policyIndex];

    require(proposalAddress == p.currentProposal,
