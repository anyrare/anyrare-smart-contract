...
contract Proposal {
  ...
  function openPolicyProposal(
    ...
    require(!policyProposals[addr].exists,
    ...
    p.closeVoteTimestamp =
        block.timestamp + g.getPolicy(policyName).voteDurationSecond;
  ...
  function votePolicyProposal(address addr, bool approve) public {
    require(policyProposals[addr].exists && policyProposals[addr].openVote,
  ...
  function processPolicyProposal(address addr) public {
    ...
    if (isVoteValid && isVoteApprove) {
      g.setPolicyByProposal(p.policyIndex, addr, p.policyWeight, p.maxWeight,
                            p.voteDurationSecond, p.minWeightOpenVote,
                            p.minWeightValidVote, p.minWeightApproveVote);
    }

    p.processResultTimestamp = block.timestamp;
  }

  function openManagerProposal(uint16 totalManager, address addr,
                               ManagerInfo memory managers) public {
    require(!managerProposals[addr].exists,
