...
contract Member {
  ...
  function setMember(
      address addr, address referral, string memory username,
      string memory thumbnail) public {
    require(
        msg.sender == addr && members[addr].referral == address(0) &&
  ...
  function updateMember(
      address addr, string memory username, string memory thumbnail) public {
    require(
        msg.sender == addr &&
        (usernames[stringToBytes32(username)] == address(0) ||
         usernames[stringToBytes32(username)] == addr));
