...
contract NFTFactory is ERC721URIStorage {
    ...
    function processAuction(uint256 tokenId) public {
        ...
        if (auction.totalBid > 0) {
            ...
            u().transferARA(address(this), nft.addr.founder, founderRoyaltyFee);
            u().transferARA(address(this), nft.addr.custodian, custodianFee);
            u().transferARA(
                address(this),
                u().getManagementFundContract(),
                platformFee
    ...
    function buyFromBuyItNow(uint256 tokenId) public payable {
        ...
        u().transferARA(
            address(this),
            u().getReferral(nft.buyItNow.owner),
            referralSellerFee
        );
    ...
    function acceptOffer(uint256 tokenId) public {
        ...
        u().transferARA(
            address(this),
            u().getReferral(nft.offer.bidder),
            referralBuyerFee
        );
        u().transferARA(
            address(this),
            u().getReferral(nft.offer.owner),
            referralSellerFee
        );
        u().transferARA(
            address(this),
