...
const deployContract = async (ethers: any, root: any) => {
  ...
  const governanceContract = await GovernanceContract.connect(root).deploy();
  const bancorFormulaContract = await BancorFormulaContract.connect(
    root
  ).deploy();
  ...
  const nftUtilsContract = await NFTUtilsContract.connect(root).deploy(
    governanceContract.address
  );
  ...
  console.log("CollateralTokenContract Addr:", collateralTokenContract.address);
