...
export const initMember = async (
  ...
  await memberContract
    ...
    .setMember(referralUser1.address, root.address);
  console.log("referralUser1");
  await memberContract
    .connect(referralUser4)
    .setMember(referralUser4.address, root.address);
  await memberContract
    .connect(referralUser2)
    .setMember(referralUser2.address, referralUser4.address);
  await expect(
    memberContract
      .connect(user3)
      .setMember(user3.address, referralUser3.address)
  ).to.be.reverted;
  ...
  await expect(
    memberContract
      .connect(referralUser1)
      .setMember(referralUser1.address, referralUser4.address)
  ).to.be.reverted;
  ...
  await memberContract
    ...
    .setMember(referralUser3.address, root.address);
  await memberContract
    .connect(referralAuditor)
    .setMember(referralAuditor.address, root.address);
  await memberContract
    .connect(referralCustodian)
    .setMember(referralCustodian.address, root.address);
  await memberContract
    .connect(user1)
    .setMember(user1.address, referralUser1.address);
  await memberContract
    .connect(user2)
    .setMember(user2.address, referralUser2.address);
  await memberContract
    .connect(user3)
    .setMember(user3.address, referralUser3.address);
  await memberContract
    .connect(user4)
    .setMember(user4.address, referralUser4.address);
  await memberContract.connect(user5).setMember(user5.address, root.address);
  await memberContract
    .connect(auditor0)
    .setMember(auditor0.address, referralAuditor.address);
  await memberContract
    .connect(auditor1)
    .setMember(auditor1.address, referralAuditor.address);
  await memberContract
    .connect(custodian0)
    .setMember(custodian0.address, referralCustodian.address);
  await memberContract
    .connect(custodian1)
    .setMember(custodian1.address, referralCustodian.address);
  await memberContract
    .connect(manager0)
    .setMember(manager0.address, root.address);
  await memberContract
    .connect(manager1)
    .setMember(manager1.address, root.address);
  await memberContract
    .connect(operation0)
    .setMember(operation0.address, root.address);
  await memberContract
    .connect(operation1)
    .setMember(operation1.address, root.address);
  console.log(
